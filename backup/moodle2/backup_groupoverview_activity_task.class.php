...
/**
 * Provides the backup_groupoverview_activity_task class.
 *
...
require_once($CFG->dirroot.'/mod/groupoverview/backup/moodle2/backup_groupoverview_stepslib.php');

/**
 * Defines the settings and steps to perform a backup of the groupoverview.
 *
 * @copyright 2016 Corinna Hertweck <corinnah.development@gmail.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_groupoverview_activity_task extends backup_activity_task {

    /**
     * Defines activity specific settings to be added to the common ones.
     *
     * The groupoverview module does not define own settings.
     *
    ...
    protected function define_my_steps() {
        $this->add_step(new backup_groupoverview_activity_structure_step('groupoverview_structure', 'groupoverview.xml'));
    }
     ...
     * the course by URLs like http://my.moodle.site/mod/groupoverview/view.php?id=42
    ...
    static public function encode_content_links($content) {
        ...
        // Link to the list of groupoverviews.
        $search = '/('.$base.'\/mod\/groupoverview\/index.php\?id\=)([0-9]+)/';
        $content = preg_replace($search, '$@groupoverviewINDEX*$2@$', $content);

        // Link to a groupoverview view by cmid.
        $search = '/('.$base.'\/mod\/groupoverview\/view.php\?id\=)([0-9]+)/';
        $content = preg_replace($search, '$@groupoverviewVIEWBYID*$2@$', $content);
